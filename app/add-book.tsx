...
export default function BooksScreen() {
  ...
  const takePicture = async () => {
    if (cameraRef.current) {
      try {
        ...
        const imageUri = `${(photo as any).uri}?t=${Date.now()}`;
  ...
  const pickFromGallery = async () => {
    try {
      ...
      if (!result.canceled && result.assets[0]) {
        ...
        const imageUri = `${result.assets[0].uri}?t=${Date.now()}`;
        setCoverImage(imageUri);
  ...
  const ImageWithFallback = ({ uri, style, fallbackIcon = "📚" }: { 
    uri: string; 
    style: any; 
    fallbackIcon?: string 
  }) => {
    ...
    if (!uri || imageError) {
      return (
        <View style={[style, { backgroundColor: '#DDD6FE', justifyContent: 'center', alignItems: 'center' }]}>
